...
canvas_collatz(colors = colorPalette("vrolik4"), background = "#dbdbdb", strands = 72, oddAngle = -0.5, evenAngle = 0.0145)
@
%Code to insert a figure [H]ere
\begin{figure}[H]
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \textbf{Solution:}
<<CreativePlot, eval=FALSE, size="scriptsize", warning=FALSE, message=FALSE>>=
set.seed(1234)
canvas_collatz(colors = colorPalette("dark2"), background = "#f2f2f2", strands = 100)
@
\begin{figure}[H]
\begin{center}
<<echo=FALSE,fig.dim=c(4,4)>>=
canvas_collatz(colors = colorPalette("dark2"))
@
...
package for \texttt{R}. \verb|\citep{aRtsy}|
...
\textbf{Solution:}
<<which, size="scriptsize">>=
?which()
solution = which(result==40)
solution
@
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % QUESTION 2d
...
\textbf{Solution:}
This approach wouldn't work because the solution is not an integer and the   numeric vector contains only integers.
